 ...
 *
 * $Source: /users/pchong/CVS/sis/espresso/unate.c,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
...
pset_family A;			/* disposes of A */
{
	 ...
	 */
	restrict1 = set_new(A->sf_size);
	min_set_ord = A->sf_size + 1;
	foreachi_set(A, i, p) {
	    if (SIZE(p) < min_set_ord) {
		set_copy(restrict1, p);
		min_set_ord = SIZE(p);
	    } else if (SIZE(p) == min_set_ord) {
		set_or(restrict1, restrict1, p);
	    }
	}
	...
	/* Check for "essential" columns */
	    ...
	    Abar = unate_complement(abs_covered_many(A, restrict1));
	    sf_free(A);
	    foreachi_set(Abar, i, p) {
		set_or(p, p, restrict1);
	    }

	/* else, recur as usual */
	} else {
	    max_i = abs_select_restricted(A, restrict1);
 ...
 *  also belongs to the set "restrict1"; weight each column of a set as
 *  1 / (set_ord(p) - 1).
 */
static int
abs_select_restricted(A, restrict1)
pset_family A;
pset restrict1;
{
    ...
    count = sf_count_restricted(A, restrict1);
